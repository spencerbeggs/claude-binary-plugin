...
import type { MetricData } from "../../src/otel/protocol.js";
...
describe("MetricHandler", () => {
	...
	describe("counter metrics", () => {
		...
		test("creates monotonic counter with explicit monotonic: true", () => {
			...
			expect(mockMeter.createCounter as ReturnType<typeof mock>).toHaveBeenCalled();
		...
		test("reuses cached monotonic counter", () => {
			...
			MetricHandler.handle({ ...data, type: { kind: "counter", value: 2 } }, mockMeter);
		...
		test("reuses cached non-monotonic counter", () => {
			...
			MetricHandler.handle(data, mockMeter);
			// Second call with same name
			MetricHandler.handle({ ...data, type: { kind: "counter", value: -1, monotonic: false } }, mockMeter);

			// UpDownCounter should only be created once
			expect(mockMeter.createUpDownCounter as ReturnType<typeof mock>).toHaveBeenCalledTimes(1);
			// But add should be called twice
	...
	describe("gauge metrics", () => {
		...
		test("reuses cached gauge", () => {
			...
			MetricHandler.handle(data, mockMeter);
			// Second call with same name
			MetricHandler.handle({ ...data, type: { kind: "gauge", value: 60 } }, mockMeter);
	...
	describe("histogram metrics", () => {
		...
		test("reuses cached histogram", () => {
			...
			MetricHandler.handle(data, mockMeter);
			// Second call with same name
			MetricHandler.handle({ ...data, type: { kind: "histogram", value: 200 } }, mockMeter);

			// Histogram should only be created once
			expect(mockMeter.createHistogram as ReturnType<typeof mock>).toHaveBeenCalledTimes(1);
		...
		test("records histogram without optional fields", () => {
			...
			// Without description/unit, the options object should not include them
