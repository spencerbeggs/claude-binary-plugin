...
import { CommandArgumentError, Commands } from "../../src/commands/runtime.js";
import type { MockEnvContext } from "../../src/testing/mocks.js";
import { MockState, TestFixtures } from "../../src/testing/mocks.js";
...
describe("Commands.parse", () => {
	test("parses and validates arguments", () => {
		...
		const result = Commands.parse(["--name=test", "--count=42"], schema);

		expect(result.name).toBe("test");
		expect(result.count).toBe(42);
	});

	test("throws CommandArgumentError for invalid arguments", () => {
		const schema = Schema.Struct({
			name: Schema.String,
		});

		expect(() => Commands.parse([], schema)).toThrow(CommandArgumentError);
	});

	test("uses default values from schema", () => {
		const schema = Schema.Struct({
			name: Schema.optionalWith(Schema.String, { default: () => "default-name" }),
		});

		const result = Commands.parse([], schema);
...
describe("Commands.emptySchema", () => {
	test("is a Schema.Struct instance", () => {
		expect(Commands.emptySchema).toBeDefined();
		expect(typeof Commands.emptySchema).toBe("function");
	});

	test("accepts empty object", () => {
		const result = Schema.decodeUnknownEither(Commands.emptySchema)({});
...
describe("Commands.ArgumentError", () => {
	...
	test("instances have exitCode 2", () => {
		...
		const error = new Commands.ArgumentError([], schema, parseError);
		expect(error.exitCode).toBe(2);
		expect(error.name).toBe("CommandArgumentError");
	});
});
...
describe("Commands.run", () => {
	...
	test("provides base state (projectDir, pluginDir, pluginEnvFile) to handler", async () => {
		...
		expect(receivedState).not.toHaveProperty("log");
		expect(receivedState).not.toHaveProperty("info");
		expect(receivedState).not.toHaveProperty("debug");
	});
