...
import { getBySessionId, getByProjectDir, registerSession } from "./SessionRegistry.js";

export const EnvResolverLive = Layer.succeed(EnvResolver, {
	getSessionEnvDir: (sessionId: string | undefined) =>
		Effect.sync(() => getBySessionId(sessionId)),

	getProjectSessionEnvDir: (projectDir: string) =>
		Effect.sync(() => getByProjectDir(projectDir)),
