...
import { isOtelEnabled } from "../otel/constants.js";

/**
 * Result data for environment validation error event.
 * @internal
 */
interface EnvValidationErrorResult {
	hookName: string;
	issueCount: number;
 ...
 *
 * 1. Define a schema using Effect Schema for type-safe validation (optional)
 ...
 *
 * const myPluginEnvSchema = Schema.Struct({
 *   MY_PLUGIN_ENABLED: Schema.Literal("true", "false"),
 *   MY_PLUGIN_API_KEY: Schema.String.pipe(Schema.minLength(1)),
 * });
 *
 * type MyPluginEnvVars = typeof myPluginEnvSchema.Type;
 *
 ...
 * import { PluginEnv } from "claude-binary-plugin";
 *
 * const lintArgsSchema = Schema.Struct({
...
/**
 * Internal interface for accessing Effect Schema struct fields.
 ...
 * - A `schema` property for Effect Schema validation (optional)
...
export abstract class PluginEnv<TOptions = Record<string, string>> {
	...
	/**
	 * Optional Effect Schema for validating environment variables.
	 * Subclasses should override this to provide type-safe validation.
	 */
	// biome-ignore lint/suspicious/noExplicitAny: Schema invariance requires `any` to allow subclass type narrowing
	protected schema?: Schema.Schema<any, any, never>;
	...
	protected loadVarsFromEnv(): TOptions | null {
		...
		this._vars = Schema.decodeUnknownSync(this.schema)(this.collectEnvVars());
		return this._vars;
	}
	...
	private collectEnvVars(): Record<string, unknown> {
		...
		const schemaWithFields = this.schema as SchemaWithFields;
		if (schemaWithFields && "fields" in schemaWithFields && typeof schemaWithFields.fields === "object") {
			for (const key of Object.keys(schemaWithFields.fields as object)) {
	 ...
	 * @param schema - Effect Schema for validating environment variables
	 ...
	 *   MY_PLUGIN_TIMEOUT_MS: Schema.NumberFromString.pipe(Schema.optionalWith({ default: () => 30000 })),
	 * });
	 *
	...
	static create<T>(
		...
		return class extends PluginEnv<T> {
			...
			protected readonly pluginName = pluginName ?? "";
	...
	static async forCommand<T extends PluginEnv, TArgs = Record<string, unknown>>(
		...
		if (params.commandName && (PluginEnv as typeof PluginEnv).commands) {
			...
			if (commandConfig?.argsSchema) {
				...
				parsedArgs = Schema.decodeUnknownSync(commandConfig.argsSchema)(argsObj) as TArgs;
			}
		}
	 ...
	 * const { state, persisted } = await MyPluginState.initializeSession({
	 *   hookName: event.name
	 * });
	 *
	...
	static async initializeSession<T extends PluginEnv>(
		...
		const debugLog = (msg: string) => {
			if (Bun.env.CLAUDE_DEBUG) {
				console.error(`[PluginEnv.initializeSession] ${msg}`);
			}
		};
		const fs = params.fs || defaultPluginEnvFileSystem;
		...
		if (instance.schema) {
			try {
				const validated = Schema.decodeUnknownSync(instance.schema)(vars);
				instance.setVars(validated);
			} catch (e) {
				debugLog(`Schema validation failed: ${e}`);
				throw e;
			}
		}
		...
		return { state: instance, persisted: persistResult };
	}
	...
	validate(): TOptions {
		...
		return Schema.decodeUnknownSync(this.schema)(this.collectEnvVars());
	}
	...
	validateWithContext(): ValidationResult<TOptions> {
		...
		const result = Schema.decodeUnknownEither(this.schema)(this.collectEnvVars());

		if (result._tag === "Right") {
			return { success: true, data: result.right };
		}

		const message = formatValidationError(result.left);
		return {
			success: false,
			error: { issues: [] },
			message,
	 ...
	 * ```
	 */
	validateOrThrow(_sessionId: string, hookName: string): void {
		const validation = this.validateWithContext();

		if (!validation.success) {
			// Emit to OTEL if enabled
			if (isOtelEnabled()) {
				// Extract path from the error message for OTEL attribution
				const path = "unknown";
				const issueCount = 1;

				// TODO(Task 15): restore OTEL emit via the new Telemetry Effect service
				void ({
					hookName,
					issueCount,
					validationPath: path,
				...
				} satisfies EnvValidationErrorResult);
			}
	 ...
	 *
	 * @param error - The validation error to format (ValidationErrorMinimal or ParseError)
	 ...
	 * try {
	 *   Schema.decodeUnknownSync(schema)({ name: 123 });
	 * } catch (error) {
	 *   console.log(PluginEnv.formatValidationError(error));
	 * }
	 * ```
	 * @public
	 */
	static formatValidationError(error: ValidationErrorMinimal | ParseResult.ParseError, maxErrors = 10): string {
		return formatValidationError(error, maxErrors);
	}
}
