...
describe("TelemetryMetrics", () => {
	...
	describe("recordHookExecution", () => {
		test("does nothing when OTEL is disabled", () => {
			...
			// Should not throw
			TelemetryMetrics.recordHookExecution(event, "test-hook", 100, true);
		});
		...
		test("records failed execution", () => {
			...
			TelemetryMetrics.recordHookExecution(event, "test-hook", 50, false);
		});
	...
	describe("recordHookDecision", () => {
		test("does nothing when OTEL is disabled", () => {
			...
			TelemetryMetrics.recordHookDecision(event, "test-hook", "allow");
		});
		...
		test("records allow decision", () => {
			...
			TelemetryMetrics.recordHookDecision(event, "test-hook", "allow", "Write");
		});
		...
		test("records block decision", () => {
			...
			TelemetryMetrics.recordHookDecision(event, "test-hook", "block", "Bash");
		});
		...
		test("records modify decision", () => {
			...
			TelemetryMetrics.recordHookDecision(event, "test-hook", "modify", "Edit");
		});
	...
	describe("recordCounter", () => {
		test("does nothing when OTEL is disabled", () => {
			...
			TelemetryMetrics.recordCounter(event, "custom.counter");
		});
		...
		test("records counter with custom value", () => {
			...
			TelemetryMetrics.recordCounter(event, "custom.counter", 5);
		});
		...
		test("records counter with attributes", () => {
			...
			TelemetryMetrics.recordCounter(event, "custom.counter", 1, { "custom.attr": "value" });
		});
	});
	...
	describe("recordHistogram", () => {
		test("does nothing when OTEL is disabled", () => {
			...
			TelemetryMetrics.recordHistogram(event, "custom.histogram", 100);
		});
		...
		test("records histogram with unit", () => {
			...
			TelemetryMetrics.recordHistogram(event, "custom.histogram", 100, "ms");
		});
		...
		test("records histogram with attributes", () => {
			...
			TelemetryMetrics.recordHistogram(event, "custom.histogram", 100, "ms", { "custom.attr": "value" });
		});
	});
	...
	describe("recordGauge", () => {
		test("does nothing when OTEL is disabled", () => {
			...
			TelemetryMetrics.recordGauge(event, "custom.gauge", 42);
		});
		...
		test("records gauge with attributes", () => {
			...
			TelemetryMetrics.recordGauge(event, "custom.gauge", 42, { "custom.attr": "value" });
		});
	});
});
